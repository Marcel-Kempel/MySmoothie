...
// Eingeloggte User direkt ins Dashboard weiterleiten.
if (is_logged_in()) {
...
// Bei POST: Eingaben validieren und User authentifizieren.
if (is_post_request()) {
    csrf_validate_or_fail($_POST['csrf_token'] ?? null);

    $authentication = service_authenticate_user($_POST);
    $formValues = $authentication['values'];
    $errors = $authentication['errors'];
    $generalError = $authentication['general_error'];

    if ($authentication['success']) {
        login_user_by_id((int) $authentication['user']['id']);
...
?>
<!-- Login-Karte mit Feldfehlern und allgemeiner Fehlermeldung -->
